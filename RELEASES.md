...
```bash
npm run release:prepare -- v0.2.1-b4
```
...
```bash
git add app.json
git commit -m "release: v0.2.1-b4"
```
...
git push origin v0.2.1-b4
```

5. Esperar workflow `.github/workflows/android-release.yml`.
6. Verificar en GitHub Release que exista el asset APK con nombre `nossa-clima-v0.2.1-b4.apk`.
